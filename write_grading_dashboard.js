  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="col-span-1 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-semibold">
            Student Submissions
          </div>
          ...
          {!selectedSession ? (
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <h2 className="text-xl font-bold mb-1">{selectedSession.candidate_name}</h2>
                <div className="text-sm text-slate-500 mb-6">{selectedSession.candidate_email}</div>
                
                <h3 className="font-semibold text-lg mb-4 border-b pb-2 dark:border-slate-800">Exam Answers</h3>
                  ...
                  <div className="space-y-8">
                    {selectedSession.responses.map((r: any, idx: number) => (
                      <div key={r.response_id} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                        <div className="bg-slate-50 dark:bg-slate-800 p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-start">
                          <div>
                          ...
                          <div className="flex flex-col items-end gap-2">
                            {r.marks_awarded !== null ? (
                              <span className="px-2 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-bold rounded">
                                Graded: {r.marks_awarded} / {r.question_points}
                              </span>
                            ) : (
                              <span className="px-2 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-xs font-bold rounded">
                                Needs Grading
                              </span>
                            )}
                          </div>
                        ...
                        <div className="p-4">
                          {r.question_type === "CODE" ? (
                            <pre className="bg-slate-950 text-slate-300 p-4 rounded-lg overflow-x-auto text-sm font-mono leading-relaxed shadow-inner">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <input 
                                ...
                                className="w-20 px-2 py-1 border rounded dark:bg-slate-800 dark:border-slate-600 text-sm" 
                                required
                              />
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <label htmlFor={\`correct-\${r.response_id}\`} className="text-sm font-medium">Mark as Correct</label>
                            </div>
                            
                            <button 
                              type="submit"
                              className="ml-auto px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-md shadow-sm transition-colors"
                            >
                              Save Grade
                            </button>
                          </form>
...
fs.writeFileSync(path.join(outPath, 'page.tsx'), CodeWriter());
